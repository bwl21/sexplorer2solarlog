...
{
  var yy, mm, dd, hh, nn, ss, vv_date=new Date(vv), ww;
  ...
  if (ii>15*24*60*60*1000)
  {
...

    {
      ...
      if (ttype==5) return(null);//You can add your own date format here
...
function _AreaResizeTo(theX0, theY0, theX1, theY1)
{
  ...
  if (!isNaN(parseInt(theY1))) this.Y1=theY1;
  if (this.X0<=this.X1) {
    ll=Math.round(this.X0);
    rr=Math.round(this.X1);
  }
  else {
    ll=Math.round(this.X1);
    rr=Math.round(this.X0);
  }
  if (this.Y0<=this.Y1) {
    tt=Math.round(this.Y0);
    bb=Math.round(this.Y1);
  }
  else {
    tt=Math.round(this.Y1);
    bb=Math.round(this.Y0);
  }
  ww=rr-ll;
  hh=bb-tt;
  if (document.all) selObj=eval("_DiagramTarget.document.all."+id);
  ...
  with (selObj.style)
  {
    if (this.Base<=tt) {
      ...
      top=tt+"px";
      width=ww+"px";
      height=Math.max(hh, this.Base-tt)+"px";
    }
    }
  if (this.Base<=tt)
  {
